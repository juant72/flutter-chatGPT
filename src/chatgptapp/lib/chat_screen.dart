...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _sendMessage() async {
    ...
    if (_isImageSearch) {
      ...
      // final request =
      //     CompleteText(prompt: message.text, model: kChatGptTurbo0301Model, maxTokens: 200);
      ...
      // // prompt: message.text, maxTokens: 200, model: kChatGptTurbo);

      // final response = chatGPT?.onChatCompletionStream(request: request);
      //---------------------
      final request = ChatCompleteText(messages: [
        Map.of({"role": "user", "content": 'Hello!'})
      ], maxToken: 200, model: kChatGptTurbo0301Model);
      ...
      Vx.log(response!.choices[0].message.toString());
